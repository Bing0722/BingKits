...
{
public:
    SQLiteStmt(SQLiteDB& db, const std::string& sql) : stmt_(NULL), db_(db.handle())
    {
    ...
    {
        check(sqlite3_bind_text(stmt_, col, val.c_str(), (int)val.size(), SQLITE_TRANSIENT));
        return *this;
    }
    ...
    sqlite3_int64 columnInt64(int col) const { return sqlite3_column_int64(stmt_, col); }
    ...
    const void* columnBlob(int col) const { return sqlite3_column_blob(stmt_, col); }
    ...
    std::string columnName(int col) const { return sqlite3_column_name(stmt_, col); }
